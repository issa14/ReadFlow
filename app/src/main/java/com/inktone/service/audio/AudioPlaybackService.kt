...
import com.inktone.service.audio.PlaybackOrchestrator.State
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.*
import javax.inject.Inject

/**
 * Service de lecture audio Media3 pour InkTone.
 *
 * Responsabilités :
 ...
 * Architecture :
 * ```
 * Commande système (BT/Notif) → MediaSession → InkTonePlayer
 *                                                    ↓
...
class AudioPlaybackService : MediaSessionService() {
    ...
    companion object {
        ...
        private const val CHANNEL_ID = "inktone_playback"
        ...
        private const val ACTION_PAUSE = "com.inktone.action.PAUSE"
        private const val ACTION_PLAY  = "com.inktone.action.PLAY"
        private const val ACTION_STOP  = "com.inktone.action.STOP"
        private const val ACTION_PREV  = "com.inktone.action.PREV"
        private const val ACTION_NEXT  = "com.inktone.action.NEXT"
    }

    @Inject lateinit var orchestrator: PlaybackOrchestrator
    @Inject lateinit var audioFocusManager: AudioFocusManager
    @Inject lateinit var onnxService: com.inktone.service.onnx.OnnxInferenceService

    private var mediaSession: MediaSession? = null
    private var inktonePlayer: InkTonePlayer? = null
    ...
    override fun onCreate() {
        ...
        inktonePlayer = InkTonePlayer(orchestrator)
        ...
        mediaSession = MediaSession.Builder(this, inktonePlayer!!)
            .setId("InkTone")
    ...
    override fun onDestroy() {
        ...
        try {
            inktonePlayer?.release()
            inktonePlayer?.cleanup()
        } catch (e: Exception) {
            Log.e(TAG, "Erreur libération InkTonePlayer: ${e.message}", e)
        }
        inktonePlayer = null
    ...
    private fun startObserving() {
        ...
        observationJob = serviceScope.launch {
            orchestrator.state.collect { state ->
                val player = inktonePlayer ?: return@collect
                ...
                when (state) {
                    is State.Idle -> {
                        player.setIdle()
                        cancelPauseTimeout()
                        updateNotification("InkTone", "Prêt", isPlaying = false)
                    }
                    is State.Loading -> {
                        cancelPauseTimeout()
                        ensureForeground()
                        val title = orchestrator.currentChapterTitle.ifEmpty {
                            orchestrator.currentBookTitle.ifEmpty { "InkTone" }
                        }
                    ...
                    is State.Paused -> {
                        ...
                        updateNotification(
                            title = orchestrator.currentChapterTitle.ifEmpty { "InkTone" },
                            subtitle = "⏸ En pause",
                            isPlaying = false
                        )
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Contrôles de lecture TTS InkTone"
                setShowBadge(false)
